...
<template>
  <div class="bg-primary min-h-screen w-screen flex flex-col items-center justify-between">
    <div class="w-full flex flex-col md:flex-row items-center justify-center p-4 relative">
      <!-- Contenedor de imágenes -->
      <div class="flex w-full max-w-7xl h-[60vh] md:h-[80vh] gap-2 items-center justify-center z-10">
        <div
          ...
          :class="[ 
            ...
            expandedIndex === index ? 'w-[80%] md:w-[60%]' : 'w-[15%] md:w-[10%] hover:bg-gray-200',
            'min-w-[40px] block'
          ]"
        >
          <!-- Imagen de fondo -->
          ...
          >
            ...
            <a :href="panel.link" class="mt-4">
              <Icon icon="mdi:plus" class="text-3xl md:text-4xl text-white hover:text-gray-300" />
            </a>
          </div>
          ...
          <div
            v-else
            class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-40 text-white text-sm md:text-xl font-bold"
          >
            {{ panel.text }}
          </div>
        </div>
      ...
      <!-- Título principal -->
      <h1 class="text-4xl md:text-7xl font-extrabold leading-tight md:leading-none md:text-right m-8 text-white text-center">
        {{ $t("hitos.title") }}
      </h1>
    </div>
    
    <!-- Icono de Scroll al final de la sección -->
